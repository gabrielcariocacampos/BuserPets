...
```bash
dk bash  # starts bash inside "projeto01" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and projeto01.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `projeto01` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The projeto01 container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv projeto01 -p python3  # creates a python3 virtualenv
