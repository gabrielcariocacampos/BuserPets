...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=projeto01.example.com
...
workon projeto01  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development projeto01 npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop projeto01
    docker rm projeto01
    docker run --name projeto01 -d --env-file /home/ubuntu/projeto01.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/projeto01:/dkdata \
        projeto01 start_web.sh
}
...
echo_green "Welcome to projeto01's dev env"
